        ...
        <div className="flex items-center flex-wrap justify-around gap-2">
          <div className="">
            <a className={styles.logo}>
              <Image alt="Logo" src={Logo} style={{maxWidth:"80%" ,margin: "auto"}} />
            </a>
          </div>
          <div className=" flex gap-5 md:gap-10">
            <div >
              <h3 className={styles.footer_links__title}>{headOne[locale]}</h3>
              <ul>
            ...
            <div >
              <h3 className={styles.footer_links__title}>{headTwo[locale]}</h3>
              <ul>
                {data.map((item, i) => {
                  return (
                    <li key={i}>
                      <Link href={`/whereToGo/${item.id}`} className={styles.footer_links}>
                        {item.name[locale]}
                      </Link>
                    </li>
                  );
