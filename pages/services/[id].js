...
import { getServices } from "@/components/useApi/dataApi";
...
if(AllData.length>0){
  AllData.map((item) =>
  ArraySelect.push({ value: item.id, label: item.name[locale] })
);
}
  const handellogin = () => {
    setLoading(true);
    const po = axios
      ...
      .catch((res) => {
        ...
        res.response.data.errors.message
          ? setErrorMessage(res.response.data.errors.message[0])
          : setErrorMessage("");
        res.response.data.errors.available_date
          ? setErrorDate(res.response.data.errors.available_date[0])
          : setErrorDate("");
        res.response.data.errors.urgency
          ? setErrorUrgency(res.response.data.errors.urgency[0])
          : setErrorUrgency("");
        res.response.data.errors.service_id
          ? setErrorservice(res.response.data.errors.service_id[0])
          : setErrorservice("");
      });
  };
  useEffect(() => {
